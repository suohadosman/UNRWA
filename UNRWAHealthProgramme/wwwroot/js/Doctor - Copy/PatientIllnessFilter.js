$(document).ready(function () {
    $('#IllnessTypeId').on('change', function () {
        var typeId = $(this).val();
        var personId = $('#personId').val();
        if (typeId == '') typeId = 0;
        if (typeId !== '') {
            $.ajax({
                url: '/Patient/GetAllPatientIllnessInType?personId=' + personId + '&typeId=' + typeId,
                success: function (result) {
                    ...
                    $.each(result, function (i, labtest) {
                        ...
                        var text1 = document.createTextNode(labtest.typeName);
                        var text2 = document.createTextNode(labtest.name)
