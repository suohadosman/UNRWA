$(document).ready(function () {
    $('#MedicineTypeId').on('change', function () {
        ...
        if (typeId !== '') {
            $.ajax({
                url: '/Doctor/PatientMedicinesInType?personId=' + personId + '&typeId=' + typeId,
                success: function (result) {
                    ...
                    $.each(result, function (i, medicine) {
                        ...
                        var text1 = document.createTextNode(medicine.medicine.name);
                        var text2 = document.createTextNode(medicine.medicine.type)
                        var text3 = document.createTextNode(medicine.previewDate)
                        var text4 = document.createTextNode(medicine.amount)
